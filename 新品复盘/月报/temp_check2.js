...
const CATEGORY_DATA = {"饰条": {"qty_3": 403.0, "rate_3": 45.2, "sku_3": 31}, "机盖及附件": {"qty_3": 138.0, "rate_3": 75.0, "sku_3": 16}, "其他": {"qty_3": 145.0, "rate_3": 75.0, "sku_3": 16}, "挡泥板": {"qty_3": 131.0, "rate_3": 40.0, "sku_3": 20}, "车身外扩件": {"qty_3": 170.0, "rate_3": 41.7, "sku_3": 12}, "车门系统": {"qty_3": 477.0, "rate_3": 50.8, "sku_3": 61}};
const ANALYST_DATA = {"俞东旭": {"sku_3": 45, "qty_3": 348.0, "rate_3": 40.0}, "胡煜星": {"sku_3": 29, "qty_3": 195.0, "rate_3": 65.5}, "章鹏": {"sku_3": 21, "qty_3": 193.0, "rate_3": 52.4}, "王偲涵": {"sku_3": 26, "qty_3": 500.0, "rate_3": 57.7}, "朱培源": {"sku_3": 35, "qty_3": 228.0, "rate_3": 54.3}};
const EXPAND_DATA = {"原开品": {"sku_3": 109, "qty_3": 1066.0, "rate_3": 56.0}, "组合件": {"sku_3": 1, "qty_3": 10.0, "rate_3": 100.0}, "拓展品": {"sku_3": 46, "qty_3": 388.0, "rate_3": 43.5}};
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  initializedCharts['pinxian']['chartCatQty'] = mkBar('chartCatQty', catLabels, [{ label: '3月销量', data: catLabels.map(function(k){return CATEGORY_DATA[k].qty_3}), backgroundColor: COLORS }]);
  initializedCharts['pinxian']['chartCatRate'] = mkBar('chartCatRate', catLabels, [{ label: '出单率(%)', data: catLabels.map(function(k){return CATEGORY_DATA[k].rate_3}), backgroundColor: COLORS }]);
  
  // 分析人维度图表
  initializedCharts['analyst'] = {};
  initializedCharts['analyst']['chartAnalystQty'] = mkBar('chartAnalystQty', Object.keys(ANALYST_DATA), [{ label: '3月销量', data: Object.values(ANALYST_DATA).map(function(v){return v.qty_3}), backgroundColor: COLORS }]);
  initializedCharts['analyst']['chartAnalystRate'] = mkBar('chartAnalystRate', Object.keys(ANALYST_DATA), [{ label: '出单率(%)', data: Object.values(ANALYST_DATA).map(function(v){return v.rate_3}), backgroundColor: COLORS }]);
  
  // 拓展类型图表
  initializedCharts['expand'] = {};
  initializedCharts['expand']['chartExpandSku'] = mkBar('chartExpandSku', Object.keys(EXPAND_DATA), [{ label: 'SKU数', data: Object.values(EXPAND_DATA).map(function(v){return v.sku_3}), backgroundColor: ['#0f3460','#e07b24','#08845a'] }]);
  initializedCharts['expand']['chartExpandRate'] = mkBar('chartExpandRate', Object.keys(EXPAND_DATA), [{ label: '出单率(%)', data: Object.values(EXPAND_DATA).map(function(v){return v.rate_3}), backgroundColor: ['#0f3460','#e07b24','#08845a'] }]);
