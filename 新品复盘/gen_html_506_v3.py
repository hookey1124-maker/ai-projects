...
# -*- coding: utf-8 -*-
"""
生成 4.30-5.6 可视化 HTML 报告 - 正确版 v3
采用清晰分离方式：Python 生成 JS 变量，HTML 模板单独维护
"""
...
# ── 提取品线数据 ───────────────────────────────────────────────────────────────
PX_DATA = [r for r in PX[1:] if r and r[0] and str(r[0]).strip() != '合计']
FX_DATA = [r for r in FX[1:] if r and r[0] and str(r[0]).strip() != '合计']
TZ_DATA = [r for r in TZ[1:] if r and r[0] and str(r[0]).strip() != '合计']
JL_DATA = [r for r in JL[1:] if r and r[0] and str(r[0]).strip() != '合计']
DY_DATA = [r for r in DY[1:] if r and r[0]]
CD_DATA = [r for r in CD[1:] if r and r[0]]
YY_DATA = [r for r in YY[1:] if r and r[0]]
PLG_DATA = [r for r in PLG[1:] if r and r[0]]
...
for r in PLP:
    ...
    if block == 'an' and lab in ['俞东旭','张潇','朱培源','王偲涵','章鹏','胡煜星']:
        ...
        PLP_TZ.append(r)
    elif block is None and lab not in ['维度','合计'] and len(PLP_TOTAL) < 5:
        PLP_TOTAL.append(r)
...
# ── JS 数据变量（用 json.dumps 保证格式正确）────────────────────────────────
def j(v):
    """将 Python 值转为 JS 字面量"""
    return json.dumps(v, ensure_ascii=False)

# 品线
PX_LAB  = j([r[0] for r in PX_DATA])
PX_SKU  = j([int(num(safe(r[1]),0)) for r in PX_DATA])
PX_NEW  = j([int(num(safe(r[2]),0)) for r in PX_DATA])
PX_SQ   = j([num(safe(r[3]),0) for r in PX_DATA])
PX_SQP  = j([num(safe(r[4]),0) for r in PX_DATA])
PX_SA   = j([num(safe(r[5]),0) for r in PX_DATA])
PX_SAP  = j([num(safe(r[6]),0) for r in PX_DATA])

# 分析人
FX_LAB  = j([r[0] for r in FX_DATA])
FX_SKU  = j([int(num(safe(r[1]),0)) for r in FX_DATA])
FX_NEW  = j([int(num(safe(r[2]),0)) for r in FX_DATA])
FX_SQ   = j([num(safe(r[3]),0) for r in FX_DATA])
FX_SQP  = j([num(safe(r[4]),0) for r in FX_DATA])
FX_SA   = j([num(safe(r[5]),0) for r in FX_DATA])
FX_SAP  = j([num(safe(r[6]),0) for r in FX_DATA])

# 拓展类型
TZ_LAB  = j([r[0] for r in TZ_DATA])
TZ_SKU  = j([int(num(safe(r[1]),0)) for r in TZ_DATA])
TZ_ORD  = j([int(num(safe(r[2]),0)) for r in TZ_DATA])
TZ_RATE = j([num(safe(r[3]),0) for r in TZ_DATA])
TZ_RP   = j([num(safe(r[4]),0) for r in TZ_DATA])
TZ_SQ   = j([num(safe(r[6]),0) for r in TZ_DATA])
TZ_SQP  = j([num(safe(r[7]),0) for r in TZ_DATA])

# 及时率
JL_LAB  = j([r[0] for r in JL_DATA])
JL_OK   = j([int(num(safe(r[1]),0)) for r in JL_DATA])
JL_8    = j([int(num(safe(r[2]),0)) for r in JL_DATA])
JL_7    = j([int(num(safe(r[3]),0)) for r in JL_DATA])
JL_RATE = j([num(safe(r[4]),0) for r in JL_DATA])
...
new Chart(document.getElementById('cPxSales'), {{
  ...
  data:{{
    labels: {PX_LAB},
    datasets:[
      {{label:'本周销量', data:{PX_SQ}, backgroundColor:P[0]}},
      {{label:'上周销量', data:{PX_SQP}, backgroundColor:P[1]}}
    ]
  }},
  options:{{ responsive:true, plugins:{{legend:{{position:'top'}}}}, scales:{{y:{{beginAtZero:true}}}} }}
}});
new Chart(document.getElementById('cPxAmt'), {{
  ...
  data:{{
    labels: {PX_LAB},
    datasets:[
      {{label:'本周销售额', data:{PX_SA}, backgroundColor:P[2]}},
      {{label:'上周销售额', data:{PX_SAP}, backgroundColor:P[3]}}
    ]
  }},
  options:{{ responsive:true, plugins:{{legend:{{position:'top'}}}}, scales:{{y:{{beginAtZero:true}}}} }}
}});
...
new Chart(document.getElementById('cFxSales'), {{
  ...
  data:{{
    labels: {FX_LAB},
    datasets:[
      {{label:'本周销量', data:{FX_SQ}, backgroundColor:P[0]}},
      {{label:'上周销量', data:{FX_SQP}, backgroundColor:P[1]}}
    ]
  }},
  options:{{ responsive:true, plugins:{{legend:{{position:'top'}}}}, scales:{{y:{{beginAtZero:true}}}} }}
}});
new Chart(document.getElementById('cFxAmt'), {{
  ...
  data:{{
    labels: {FX_LAB},
    datasets:[
      {{label:'本周销售额', data:{FX_SA}, backgroundColor:P[2]}},
      {{label:'上周销售额', data:{FX_SAP}, backgroundColor:P[3]}}
    ]
  }},
  options:{{ responsive:true, plugins:{{legend:{{position:'top'}}}}, scales:{{y:{{beginAtZero:true}}}} }}
}});
...
new Chart(document.getElementById('cTzRate'), {{
  ...
  data:{{
    labels: {TZ_LAB},
    datasets:[
      {{label:'本周出单率(%)', data:{TZ_RATE}, backgroundColor:P[0]}},
      {{label:'上周出单率(%)', data:{TZ_RP}, backgroundColor:P[1]}}
    ]
  }},
...
new Chart(document.getElementById('cTzSales'), {{
  ...
  data:{{
    labels: {TZ_LAB},
    datasets:[
      {{label:'本周销量', data:{TZ_SQ}, backgroundColor:P[2]}},
      {{label:'上周销量', data:{TZ_SQP}, backgroundColor:P[3]}}
    ]
  }},
  options:{{ responsive:true, plugins:{{legend:{{position:'top'}}}}, scales:{{y:{{beginAtZero:true}}}} }}
}});
...
new Chart(document.getElementById('cJlRate'), {{
  ...
  data:{{
    labels: {JL_LAB},
    datasets:[
      {{label:'及时率(%)', data:{JL_RATE}, backgroundColor:P.slice(0,{len(JL_DATA)})}}
    ]
  }},
...
with open(DST, 'w', encoding='utf-8') as f:
    f.write(''.join(html_parts) + js_code)

print('\nDone!', DST)
print('Size:', len(''.join(html_parts) + js_code), 'bytes')
