...
all_detail = temp_data['all_detail']
...
for batch in ['1月', '2月', '3月']:
    items = [x for x in all_detail if x['batch'] == batch]
    total_qty = sum(x['qty'] for x in items)
    total_comp = sum(x['comp'] for x in items)
    ordered = len([x for x in items if x['order'] == 'Y'])
    unorder = len([x for x in items if x['order'] == 'N'])
    not_order = len([x for x in items if x['order'] == '未出单'])
    rate = (ordered / len(items) * 100) if items else 0
    share = (total_qty / (total_qty + total_comp) * 100) if (total_qty + total_comp) > 0 else 0
    ...
    if batch == '1月':
        ...
        prev_items = [x for x in all_detail if x['batch'] == '1月']
        prev_qty = sum(x['qty'] for x in prev_items)
        prev_rate = (len([x for x in prev_items if x['order'] == 'Y']) / len(prev_items) * 100) if prev_items else 0
        qty_hb = total_qty - prev_qty
        rate_hb = rate - prev_rate
    else:  # 3月
        prev_items = [x for x in all_detail if x['batch'] == '2月']
        prev_qty = sum(x['qty'] for x in prev_items)
        prev_rate = (len([x for x in prev_items if x['order'] == 'Y']) / len(prev_items) * 100) if prev_items else 0
        qty_hb = total_qty - prev_qty
        rate_hb = rate - prev_rate
    ...
    batch_stats[batch] = {
        ...
        'share': round(share, 1),
        'qty_hb': qty_hb,
        'rate_hb': round(rate_hb, 1) if rate_hb is not None else None
    }

# 品类统计
cat_stats = {}
for cat in set(x['cat'] for x in all_detail if x['cat']):
    items = [x for x in all_detail if x['cat'] == cat]
    qty_3 = sum(x['qty'] for x in items)
    ordered = len([x for x in items if x['order'] == 'Y'])
    rate = (ordered / len(items) * 100) if items else 0
    cat_stats[cat] = {
        'qty_3': round(qty_3, 1),
...
# 分析人统计
analyst_stats = {}
for analyst in set(x['analyst'] for x in all_detail if x['analyst']):
    items = [x for x in all_detail if x['analyst'] == analyst]
    qty_3 = sum(x['qty'] for x in items)
    ordered = len([x for x in items if x['order'] == 'Y'])
    rate = (ordered / len(items) * 100) if items else 0
...
# 拓展类型统计
expand_stats = {}
for expand in set(x['expand'] for x in all_detail if x['expand']):
    items = [x for x in all_detail if x['expand'] == expand]
    qty_3 = sum(x['qty'] for x in items)
    ordered = len([x for x in items if x['order'] == 'Y'])
    rate = (ordered / len(items) * 100) if items else 0
    expand_stats[expand] = {
...
for item in all_detail:
    ...
    market_dist[status] = market_dist.get(status, 0) + 1

# ========== 生成HTML ==========
# 使用字符串拼接而不是f-string来处理JS代码
...
<div class="header">
  <h1>🚀 新品月报 · 2026年3月（完整版）</h1>
  <div class="subtitle">统计周期：2026年3月 | 在跟SKU总数：''' + str(len(all_detail)) + ''' | 生成时间：''' + datetime.now().strftime('%Y-%m-%d %H:%M') + '''</div>
</div>
...
<!-- KPI总览 -->
<div class="kpi-grid">
  <div class="kpi-card"><div class="val">''' + str(len(all_detail)) + '''</div><div class="label">在跟SKU总数</div></div>
  <div class="kpi-card green"><div class="val">''' + str(len([x for x in all_detail if x['order']=='Y'])) + '''</div><div class="label">3月已出单SKU</div></div>
  <div class="kpi-card red"><div class="val">''' + str(len([x for x in all_detail if x['order']!='Y'])) + '''</div><div class="label">3月未出单SKU</div></div>
  <div class="kpi-card orange"><div class="val">''' + str(batch_stats['3月']['rate']) + '''%</div><div class="label">3月出单率</div><div class="hb">''' + (f"环比 {batch_stats['3月']['rate_hb']:+.1f}%" if batch_stats['3月']['rate_hb'] else "") + '''</div></div>
  <div class="kpi-card"><div class="val">''' + str(int(batch_stats['3月']['qty'])) + '''</div><div class="label">3月总销量</div><div class="hb">''' + (f"环比 {int(batch_stats['3月']['qty_hb']):+d}" if batch_stats['3月']['qty_hb'] is not None else "") + '''</div></div>
  <div class="kpi-card"><div class="val">''' + str(int(batch_stats['3月']['share'])) + '''%</div><div class="label">3月市占比</div></div>
</div>
''')
...
<thead><tr><th>品类</th><th>SKU数</th><th>3月销量</th><th>3月出单率</th></tr></thead>
...
<h4>各分析人3月绩效统计</h4>
<table class="data-table">
<thead><tr><th>分析人</th><th>SKU数</th><th>3月销量</th><th>3月出单率</th></tr></thead>
...
<h4>各拓展类型3月绩效统计</h4>
<table class="data-table">
<thead><tr><th>拓展类型</th><th>SKU数</th><th>3月销量</th><th>3月出单率</th></tr></thead>
...
# 低占比新品
low_1 = len([x for x in low_share if x['batch']=='1月'])
low_2 = len([x for x in low_share if x['batch']=='2月'])
low_3 = len([x for x in low_share if x['batch']=='3月'])
all_1 = len([x for x in all_detail if x['batch']=='1月'])
all_2 = len([x for x in all_detail if x['batch']=='2月'])
all_3 = len([x for x in all_detail if x['batch']=='3月'])
  ...
  <div class="section">
    ...
    <div style="margin-bottom:10px;">
      <select id="low-share-batch" onchange="renderLowShare()" style="padding:6px 12px;border-radius:6px;border:1px solid #ddd;font-size:13px;">
        ...
        <option value="1">1月新品（{low_1}个SKU）</option>
        <option value="2">2月新品（{low_2}个SKU）</option>
        <option value="3">3月新品（{low_3}个SKU）</option>
      </select>
    </div>
...
<div id="section-detail" class="section-wrap" style="display:none">
  <div class="section">
    <h3>📋 全量SKU明细（共{len(all_detail)}个SKU）</h3>
    <p style="font-size:12px;color:#888;margin-bottom:10px;">绿色背景行为3月8日已出单SKU</p>
    <div style="margin-bottom:10px;">
      <select id="detail-batch" onchange="renderDetailTable()" style="padding:6px 12px;border-radius:6px;border:1px solid #ddd;font-size:13px;">
        ...
        <option value="1">1月新品（{all_1}个SKU）</option>
        <option value="2">2月新品（{all_2}个SKU）</option>
        <option value="3">3月新品（{all_3}个SKU）</option>
      </select>
    </div>
    <div class="table-wrap">
    <table class="data-table">
...
<script>
// ========== 数据定义 ==========
const ALL_DATA = ''' + json.dumps(all_detail, ensure_ascii=False) + ''';
const LOW_SHARE_DATA = ''' + json.dumps(low_share, ensure_ascii=False) + ''';
const BATCH_DATA = ''' + json.dumps(batch_stats, ensure_ascii=False) + ''';
const CATEGORY_DATA = ''' + json.dumps(cat_stats, ensure_ascii=False) + ''';
const ANALYST_DATA = ''' + json.dumps(analyst_stats, ensure_ascii=False) + ''';
const EXPAND_DATA = ''' + json.dumps(expand_stats, ensure_ascii=False) + ''';
const MARKET_DATA = ''' + json.dumps(market_dist, ensure_ascii=False) + ''';
...
function mkBar(id, labels, datasets) {
  ...
  if (!ctx) return;
  new Chart(ctx, {type: 'bar', data: { labels: labels, datasets: datasets }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: datasets.length > 1, position: 'top', labels: { font: { size: 10 } } } }, scales: { x: { ticks: { font: { size: 10 }, maxRotation: 30 } }, y: { ticks: { font: { size: 10 } }, beginAtZero: true } }}});
}
function mkLine(id, labels, datasets) {
  var ctx = document.getElementById(id);
  if (!ctx) return;
  new Chart(ctx, {type: 'line', data: { labels: labels, datasets: datasets }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: datasets.length > 1, position: 'top', labels: { font: { size: 10 } } } }, scales: { x: { ticks: { font: { size: 10 } } }, y: { ticks: { font: { size: 10 } } }}});
}
function mkPie(id, labels, data) {
  var ctx = document.getElementById(id);
  if (!ctx) return;
  new Chart(ctx, {type: 'doughnut', data: { labels: labels, datasets: [{ data: data, backgroundColor: COLORS, borderWidth: 1 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: { font: { size: 10 }, boxWidth: 12 } } }}});
}
...
mkBar('chartCategoryQty', catLabels, [{ label: '3月销量', data: catLabels.map(function(k){return CATEGORY_DATA[k].qty_3}), backgroundColor: COLORS }]);
mkBar('chartCategoryRate', catLabels, [{ label: '出单率(%)', data: catLabels.map(function(k){return CATEGORY_DATA[k].rate_3}), backgroundColor: COLORS }]);
mkBar('chartExpand', Object.keys(EXPAND_DATA), [{ label: '3月销量', data: Object.values(EXPAND_DATA).map(function(v){return v.qty_3}), backgroundColor: ['#0f3460','#e07b24','#08845a'] }]);
mkBar('chartAnalyst', Object.keys(ANALYST_DATA), [{ label: '3月销量', data: Object.values(ANALYST_DATA).map(function(v){return v.qty_3}), backgroundColor: COLORS }]);
...
function renderCatTable() {
  ...
  for (var cat in CATEGORY_DATA) {
    ...
    html += '<tr><td style="text-align:left;font-weight:600">' + cat + '</td><td>' + d.sku_3 + '</td><td>' + d.qty_3 + '</td><td>' + d.rate_3 + '%</td></tr>';
  }
...
function renderAnalystTable() {
  ...
  for (var name in ANALYST_DATA) {
    ...
    html += '<tr><td style="text-align:left;font-weight:600">' + name + '</td><td>' + d.sku_3 + '</td><td>' + d.qty_3 + '</td><td class="highlight">' + d.rate_3 + '%</td></tr>';
  }
...
function renderExpandTable() {
  ...
  for (var exp in EXPAND_DATA) {
    ...
    html += '<tr><td style="text-align:left;font-weight:600">' + exp + '</td><td>' + d.sku_3 + '</td><td>' + d.qty_3 + '</td><td>' + d.rate_3 + '%</td></tr>';
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  mkBar('chartCatQty', catLabels, [{ label: '3月销量', data: catLabels.map(function(k){return CATEGORY_DATA[k].qty_3}), backgroundColor: COLORS }]);
  mkBar('chartCatRate', catLabels, [{ label: '出单率(%)', data: catLabels.map(function(k){return CATEGORY_DATA[k].rate_3}), backgroundColor: COLORS }]);
  mkBar('chartAnalystQty', Object.keys(ANALYST_DATA), [{ label: '3月销量', data: Object.values(ANALYST_DATA).map(function(v){return v.qty_3}), backgroundColor: COLORS }]);
  mkBar('chartAnalystRate', Object.keys(ANALYST_DATA), [{ label: '出单率(%)', data: Object.values(ANALYST_DATA).map(function(v){return v.rate_3}), backgroundColor: COLORS }]);
  mkBar('chartExpandSku', Object.keys(EXPAND_DATA), [{ label: 'SKU数', data: Object.values(EXPAND_DATA).map(function(v){return v.sku_3}), backgroundColor: ['#0f3460','#e07b24','#08845a'] }]);
  mkBar('chartExpandRate', Object.keys(EXPAND_DATA), [{ label: '出单率(%)', data: Object.values(EXPAND_DATA).map(function(v){return v.rate_3}), backgroundColor: ['#0f3460','#e07b24','#08845a'] }]);
  ...
  mkLine('chartOrderRate', ['1月新品', '2月新品', '3月新品'], [{label: '出单率(%)', data: [BATCH_DATA['1月'].rate, BATCH_DATA['2月'].rate, BATCH_DATA['3月'].rate], borderColor: '#0f3460', backgroundColor: 'rgba(15,52,96,0.1)', fill: true, tension: 0.3}]);
...
# 写入文件
with open('新品月报_2026年3月_完整版.html', 'w', encoding='utf-8') as f:
    f.write('\n'.join(html_parts))

print(f'HTML生成成功！全量明细: {len(all_detail)}条，低占比新品: {len(low_share)}条')
